...
def add_camera_rgb(width=2048, height=1536, audio=True, envirosky=False, flycam=False, player='cameras', playerCamera='cameras/cameraRGB'):
	...
	if audio:
		send_data(['{} ADD AudioListener'.format(playerCamera)])
	if flycam:
		send_data(['{} ADD FlyCamera'.format(playerCamera)])
	if envirosky:
		send_data([
...
def camera_rgb_pp_random(playerCamera='cameras/cameraRGB'):
	...
	# tonemapping
	send_data(['{} SET ScionEngine.ScionPostProcess tonemappingMode {}'.format(playerCamera, random.choice(tonemappings))])


def add_camera_seg(width=2048, height=1536, segment="Car"):
	send_data([
...
def take_seg_snapshot( lst ):
	for l in lst:
		r = send_data('{} GET Segmentation.Segmentation boundingBoxes'.format(l))
		if (len(r) > 1):
...
def spawn_radius_generic(types=[], scale=[1,1,1], innerradius=0, radius=500, position=[0,0,0], rotation=[0,0,0], limit=50, segmentation_class="None", orbit=False, stick_to_ground=False, collision_check=True, suffix=""):
	...
	for t in types:
		...
		send_data([
			...
			'spawner/{} SET RandomProps.PropArea tags "{}"'.format(n, t),
		...
		if segmentation_class != "None":
			send_data([
				...
				'spawner/{} SET Segmentation.ClassGroup itemsClass {}'.format(n, segmentation_class)
			])
		
		if orbit == True:
			send_data('cameras SET Orbit target spawner/{}'.format(n))
...
def spawn_flat_grid(types=[], size=[1000,1000], position=[0,0,0], scale=[1,1,1]):
	for t in types:
		...
		send_data([
			...
			'spawner/{} SET Transform position ({} {} {})'.format(n, position[0], position[1], position[2]),
			'spawner/{} SET RandomProps.FillGrid tags {}'.format(n, n),
...
def spawn_parking_lot( limit, fixed=False, dist_h=8, dist_v=3, dist_lim=30, p_x=-15, p_z=-30, p_y=-5 ):
		...
		while k < limit:
			...
			send_data([
				...
				'cars/car_{} SET Segmentation.ClassGroup itemsClass Car'.format(k),
...
def spawn_misc_objs(destroy=False):
	...
	# spawn_flat_grid(['ground'], scale=[3,3,3], position=[0,0,-.5])
	# spawn_radius_generic(['decal'], limit=350, radius=50, innerradius=0, collision_check=False, position=[0,10,0])
	# spawn_radius_generic(['trafficlight'], limit=100, radius=50, innerradius=30)
	
	spawn_radius_generic(['tree'], collision_check=False, limit=random.randint(100, 350), radius=80, innerradius=random.randint(30, 50), position=[0,10,0])
	spawn_radius_generic(['sign'], limit=random.randint(80, 250), radius=random.randint(15, 40), innerradius=0, position=[0,10,0])
	spawn_radius_generic(['animal'], limit=random.randint(150, 250), radius=random.randint(40, 80), innerradius=0, position=[0,10,0])
	spawn_radius_generic(['building'], limit=random.randint(50, 150), radius=335, innerradius=random.randint(80, 100), position=[0,10,0])
	spawn_radius_generic(['car'], limit=random.randint(50, 150), radius=50, innerradius=0, segmentation_class="Car", orbit=True, position=[0,10,0])

	# for i in range(0,9):
	# 	spawn_radius_generic(['ground'], suffix='_{}'.format(i), limit=random.randint(50, 100), radius=75, innerradius=0, scale=[2,2,2], position=[0,i,0], collision_check=False)
	# spawn_radius_generic(['ground'], suffix='_0', limit=random.randint(3, 10), radius=75, innerradius=0, scale=[2,2,2], position=[0,0,0], collision_check=False)
	spawn_radius_generic(['ground'], suffix='_0', limit=3, radius=75, innerradius=0, scale=[1,1,1], position=[0,0,0], collision_check=False)
...
def spawn_drone_objs(destroy=False, ground_limit=204, dist_h=120, dist_v=120, dist_lim=1000, p_x=-20, p_z=-1000, p_y=0):
	if destroy == True:
		...
		# for i in range(0,2):
		# 	spawn_radius_generic(['ground'], suffix='_{}'.format(i), limit=5, radius=100, innerradius=0, scale=[2,2,2], position=[0,i,0], collision_check=False)
	
	spawn_radius_generic(['tree'], collision_check=False, limit=random.randint(50,200), radius=80, innerradius=20, position=[0,0,0])
	spawn_radius_generic(['building'], limit=random.randint(50,150), radius=335, innerradius=100, position=[0,0,0])
	spawn_radius_generic(['bird'], limit=random.randint(25,100), radius=random.randint(80,110), innerradius=0, position=[0,random.randint(15,95),0])
	spawn_radius_generic(['car'], limit=random.randint(5, 25), radius=50, innerradius=5, position=[0,0,0])
	spawn_radius_generic(['drone'], limit=random.randint(80,200), radius=random.randint(30,50), innerradius=0, position=[0,0,0], segmentation_class="Car")
	# spawn_radius_generic(['+drone,+white'], limit=random.randint(10,50), radius=random.randint(30,50), innerradius=0, position=[0,0,0], segmentation_class="Car")
...
def spawn_drone_objs_alt(destroy=False, ground_limit=204, dist_h=120, dist_v=120, dist_lim=1000, p_x=-20, p_z=-1000, p_y=0):
	...
	spawn_radius_generic(['tree'], limit=random.randint(600,600), radius=80, innerradius=30, position=[0,0,0], collision_check=False)
	spawn_radius_generic(['building'], limit=random.randint(10,10), radius=335, innerradius=100, position=[0,0,0], collision_check=False)
	spawn_radius_generic(['bird'], limit=random.randint(10,10), radius=random.randint(80,110), innerradius=0, position=[0,random.randint(15,95),0])
	spawn_radius_generic(['car'], limit=random.randint(5, 10), radius=50, innerradius=5, position=[0,0,0])
...
def spawn_animals_objs(destroy=False):
	...
	spawn_radius_generic(['tree'], collision_check=False, stick_to_ground=False, limit=random.randint(150, 400), radius=80, innerradius=random.randint(30, 50), position=[0,5,0])
	spawn_radius_generic(['animal'], stick_to_ground=False, limit=random.randint(200, 350), radius=random.randint(40, 80), innerradius=0, position=[0,5,0], orbit=True)
...
if platform.system() == 'Windows':
	...
	parser.add_argument('-l', '--local_output', default='E:\\tmp\\', action=readable_dir, help='Defines local output path for recordings, json exports, etc; This path is relative to the machine running this script, defaults to E:\\tmp\\', dest='local_path')
else:
...
if run:
	...
	if demo == 0:
		...
		while k < cars_limit:
			send_data([
				...
				'mycar_{} SET Segmentation.ClassGroup itemsClass Car'.format(k),
	...
	# create a parking lot with all props and take images on helicopter perspective
		...
		if skip_setup == False:
			...
			spawn_radius_generic(['tree'], collision_check=False, limit=250, radius=80, innerradius=30, position=[0,10,0])
			spawn_radius_generic(['sign'], limit=100, radius=35, innerradius=0, position=[0,10,0])
			spawn_radius_generic(['animal'], limit=300, radius=50, innerradius=0, position=[0,10,0])
			spawn_radius_generic(['building'], limit=100, radius=335, innerradius=80, position=[0,10,0])
			
			for i in range(0,9):
				spawn_radius_generic(['ground'], suffix='_{}'.format(i), limit=50, radius=50, innerradius=0, scale=[2,2,2], position=[0,i,0], collision_check=False)
	...
	# forest scene test
		...
		if skip_setup == False:
			...
			# spawn_radius_generic(['car'], limit=10, radius=50, innerradius=0, segmentation_class="Car", orbit=True, position=[0,10,0])
			# spawn_radius_generic(['desert'], orbit=True, limit=1, radius=65, innerradius=0, position=[0,0,0], collision_check=False)
	...
	# POC alternate scene - without segmentation and animals
		...
		if skip_setup == False:
			...
			spawn_radius_generic(['tree'], collision_check=False, limit=50, radius=80, innerradius=30, position=[0,10,0])
			spawn_radius_generic(['sign'], limit=10, radius=35, innerradius=0, position=[0,10,0])
			# spawn_radius_generic(['animal'], limit=300, radius=50, innerradius=0, position=[0,10,0])
			spawn_radius_generic(['building'], limit=10, radius=335, innerradius=80, position=[0,10,0])
			spawn_radius_generic(['car'], limit=10, radius=50, innerradius=0, segmentation_class="Car", orbit=True, position=[0,10,0])
			
			for i in range(0,9):
				spawn_radius_generic(['ground'], suffix='_{}'.format(i), limit=3, radius=50, innerradius=0, scale=[2,2,2], position=[0,i,0], collision_check=False)
			# spawn_radius_generic(['ground'], suffix='_{}'.format(i), limit=1, radius=50, innerradius=0, scale=[2,2,2], position=[0,0,0], collision_check=False)
		...
		if skip_setup == False:
			...
			for i in range(0,9):
				spawn_radius_generic(['ground'], suffix='_{}'.format(i), limit=5, radius=50, innerradius=0, scale=[2,2,2], position=[0,i,0], collision_check=False)
		...
		if skip_setup == False:
			...
			add_camera_rgb_pp('EnviroFX', False)
			send_data([
				'cameras/cameraRGB SET UnityEngine.PostProcessing.PostProcessingBehaviour profile Thermal',
				'cameras/cameraRGB ADD Thermal.Thermal'
			])
			...
			# spawn_radius_generic(['animal'], limit=300, radius=50, innerradius=0, position=[0,10,0])
			spawn_radius_generic(['building'], limit=10, radius=335, innerradius=80, position=[0,10,0])
			spawn_radius_generic(['car, +thermal'], limit=10, radius=50, innerradius=0, segmentation_class="Car", orbit=True, position=[0,10,0])
	...
	# scene basic setup
		...
		spawn_radius_generic(types=['building', 'prop', 'sign'], position=pos, limit=100, radius=100, innerradius=50)
