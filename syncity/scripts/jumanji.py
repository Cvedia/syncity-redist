...
def args(parser):
    try:
        ...
        pass
    try:
        parser.add_argument('--random_texture_source', default=None,
                            help='Defines a random texture source, if not set will not randomize textures')
    except:
        pass
...
def run():
    loop = 0
    mycams = ['Camera/Thermal', 'Camera/Segmentation', 'Camera/RGB']
    ...
    if settings.random_texture_source != None:
        for r in randomTextures:
            common.sendData([
                '"{}" ADD ObjectTagging.Spawners.RandomizeMainTexture Thermal.Spawners.RandomizeThermalTexture'.format(
                    r),
                '"{}" EXECUTE ObjectTagging.Spawners.RandomizeMainTexture GetImagesFromDisk "{}"'.format(r,
                                                                                                         settings.random_texture_source),
                '"{}" EXECUTE Thermal.Spawners.RandomizeThermalTexture GetImagesFromDisk "{}"'.format(r,
                                                                                                      settings.random_texture_source)
            ])
    ...
    while loop < settings.loop_limit:
        common.sendData([
            ...
            '[RandomProps.Spawner] ShuffleAll "Cars" "Trees" "Signs" "Grounds" "Trafficlights" "Humans"',
            '[RandomProps.Spawner] ShuffleAll "Bicycles"'
        ])
